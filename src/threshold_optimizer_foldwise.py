...
def main(configs, folds, valid_df):
    ...
    for c_dict, fold in tqdm(zip(configs, folds), total=len(configs)):
        output_conf = []
        ...
        # output_conf -> (len(valid_df), len(clip), 264)
        outputs.append(output_conf)
    # output_conf -> (len(config), len(valid_df), len(clip), 264)
    pred_clips = []
    for di in range(len(valid_df)):
        o = np.zeros_like(outputs[0][di])
        for ci in range(len(outputs)):
            o = o + outputs[ci][di]
        o = o / len(outputs)
        pred_clips.append(o)
    trues = np.array([BIRD_CODE[i] for i in valid_df.ebird_code.values])
...
if __name__ == __name__:
    ...
    parser.add_argument('--target_folds', nargs='+',
                        type=int, default=[0, 1, 2, 3, 4])
    parser.add_argument(
        '--valid_path', default="../input/birdsong-recognition/valid_pl2.csv")
    ...
    if len(configs) == 1 and len(args.target_folds) > 0:
        configs = configs * len(args.target_folds)
    assert len(configs) == len(args.target_folds)
    valid_df = pd.read_csv(args.valid_path)
    main(configs, args.target_folds, valid_df)
