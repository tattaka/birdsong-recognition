...
class DFT(DFTBase):
    ...
    def dft(self, x_real, x_imag):
        ...
        """
        z_real = torch.matmul(x_real, self.W_real) - \
            torch.matmul(x_imag, self.W_imag)
        z_imag = torch.matmul(x_imag, self.W_real) + \
            torch.matmul(x_real, self.W_imag)
    ...
    def idft(self, x_real, x_imag):
        ...
        Returns:
          ...
          z_imag: (n,), output imag part
        """
        z_real = torch.matmul(x_real, self.inv_W_real) - \
            torch.matmul(x_imag, self.inv_W_imag)
        z_imag = torch.matmul(x_imag, self.inv_W_real) + \
            torch.matmul(x_real, self.inv_W_imag)
    ...
    def irdft(self, x_real, x_imag):
        ...
        x_imag = torch.cat(
            (x_imag, -1. * flip_x_imag[..., 1: n_rfft - 1]), dim=-1)
...
class STFT(DFTBase):
    ...
    def forward(self, input):
        ...
        if self.center:
            x = F.pad(x, pad=(self.n_fft // 2, self.n_fft // 2),
                      mode=self.pad_mode)
...
class ISTFT(DFTBase):
    ...
    def forward(self, real_stft, imag_stft, length):
        ...
        full_imag_stft = torch.cat(
            (imag_stft, - self.reverse(imag_stft)), dim=1)
        """(1, n_fft, time_steps)"""

        # IDFT
        s_real = self.conv_real(full_real_stft) - \
            self.conv_imag(full_imag_stft)
...
class LogmelFilterBank(nn.Module):
    ...
    def power_to_db(self, input):
        ...
        ref_value = self.ref
        log_spec = 10.0 * \
            torch.log10(torch.clamp(input, min=self.amin, max=np.inf))
...
class DropStripes(nn.Module):
    ...
    def transform_slice(self, e, total_width):
        ...
        for _ in range(self.stripes_num):
            ...
            bgn = torch.randint(low=0, high=total_width -
                                distance, size=(1,))[0]
